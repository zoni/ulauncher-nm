...
class NMExtension(Extension):
    ...
    def list_settings(self, query):
        ...
        if (state[3] == "enabled") and (query in profiles[0]["name"].lower()):
            items.append(create_item(profiles[0]["name"], profiles[0]["desc"], "wifi-disable",
                                     {"mod": "settings", "name": "disable_wifi"}))
        if (state[3] == "disabled") and (query in profiles[1]["name"].lower()):
            items.append(create_item(profiles[1]["name"], profiles[1]["desc"], "wifi-enable",
                                     {"mod": "settings", "name": "enable_wifi"}))
        if (state[0] != "asleep") and (query in profiles[2]["name"].lower()):
            items.append(create_item(profiles[2]["name"], profiles[2]["desc"], "network-disable",
                                     {"mod": "settings", "name": "disable_net"}))
        if (state[0] == "asleep") and (query in profiles[3]["name"].lower()):
            items.append(create_item(profiles[3]["name"], profiles[3]["desc"], "network-enable",
                                     {"mod": "settings", "name": "enable_net"}))

        if query in profiles[4]["name"].lower():
            items.append(create_item(profiles[4]["name"], profiles[4]["desc"], "wifi-rescan",
                                     {"mod": "settings", "name": "rescan_wifi"}))
